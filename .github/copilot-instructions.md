...
When generating git commit messages, always use Conventional Commits format:
...
- Always include a concrete `scope` based on the main area changed, such as `home`, `settings`, `layout`, `theme`, `i18n`, `api`, `storage`, or `showcase`
- Keep the subject short, specific, and lowercase unless a proper noun requires capitalization
- Do not use vague commit messages like `Update`, `Changes`, `Fix`, or `WIP`
- Output only the final commit subject line when asked to generate a commit message
